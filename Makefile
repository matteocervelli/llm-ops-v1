...
mlx-27b: ## Start MLX server on Studio with Qwen3.6-27B (tmux session)
	ssh $(STUDIO) " \
	  tmux kill-session -t $(MLX_SESSION_27B) 2>/dev/null; \
	  tmux new-session -d -s $(MLX_SESSION_27B) \
	    'source $(MLX_VENV) && \
	...
	@echo "Logs: ssh $(STUDIO) tmux attach -t $(MLX_SESSION_27B)"

mlx-35b: ## Start MLX server on Studio with Qwen3.6-35B-A3B (tmux session)
	ssh $(STUDIO) " \
	  tmux kill-session -t $(MLX_SESSION_35B) 2>/dev/null; \
	  tmux new-session -d -s $(MLX_SESSION_35B) \
	    'source $(MLX_VENV) && \
	     HF_HOME=$(MLX_HF_HOME) mlx_lm.server \
	...
	@echo "Logs: ssh $(STUDIO) tmux attach -t $(MLX_SESSION_35B)"

mlx-stop: ## Stop any MLX server running on Studio
	ssh $(STUDIO) " \
	  tmux kill-session -t $(MLX_SESSION_27B) 2>/dev/null; \
	  tmux kill-session -t $(MLX_SESSION_35B) 2>/dev/null; \
