...
- **Primitive coperte (in sequenza):**
  ...
  **Gap check su nostro setup (2 min):** Due punti deboli che queste best practice illuminano:
  ...
  2. Configurazione — `settings.json` + permission modes vs `config.toml` + approval policy/sandbox. **Fast mode (15 sec):** `/fast` attiva la modalità ad alta velocità — stesso Opus, 2.5x più veloce, costo più alto ($30/$150 MTok). Utile in iterazione rapida e debugging live. Non è un modello diverso. Research preview — pricing può cambiare. _"Se state facendo pair programming con Claude e la latenza vi frena, `/fast` esiste. Per task autonomi lunghi, non serve."_ **`--bare` (10 sec):** `claude --bare` bypassa hooks/plugin/memoria — startup da ~3-4s a ~400ms. Per query rapide una-tantum dove non serve il harness completo. **LSP tool (30 sec):** `enabledPlugins` in `settings.json` attiva i language server per Python (`pyright-lsp`) e TypeScript (`typescript-lsp`). In pratica: Claude trova definizioni di funzioni in ~50ms invece di grep su centinaia di file; traccia la call hierarchy reale; cattura type errors subito dopo ogni edit. Il nostro setup ce l'ha già attivo — mostrare `~/.claude/settings.json > enabledPlugins`. Calibrazione sul post virale ("600x faster"): la direzione è corretta, il numero è marketing — il guadagno reale è precisione nella navigazione e risparmio di token su ricerche sbagliate.
  3. **Context rot (5 min):** fenomeno: sessione lunga → più token di input → costo crescente + output quality che degrada. Dati di ricerca (paper/benchmark). Exit prematura. Motivazione per tutto il sistema di hooks e memoria. **Token budget math (1 min aggiuntivo, dentro context rot):** il context window da 200K non è tutto disponibile per il tuo lavoro. Struttura reale: ~15-20K fissi (system prompt, descriptor di tutte le skill attive, definizioni MCP, stato LSP) + ~5-10K semi-fissi (CLAUDE.md, memory) + ~160-180K dinamici (conversazione, file, output tool). Il numero che sorprende: ogni MCP server espone 20-30 definizioni tool a ~200 token ciascuna — 5 server = **~25K token fissi = 12.5% del context consumato solo per le definizioni, prima di scrivere una riga di codice**. Se hai 6 MCP server connessi e non li usi, stai regalando un ottavo del context window a schemi JSON. **→ 4 tier di compaction (30 sec):** Claude Code ha 4 livelli interni: **Snip** (truncation storica preservando la cache), **Micro** (clearing tool result datati), **Full** (summarization — questo è `/compact`), **Reactive Collapse** (emergenza su errore 413). Variabile live-dimostrabile: `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=70` — scatta Full al 70% invece del default ~95%. Previene il degrado tardi nelle sessioni lunghe.
  ...
  5. **MCP vs CLI tools (3 min):** MCP = protocollo per dare tool esterni agli agenti (database, API, filesystem). CLI tools = comandi bash. Le skill del setup usano entrambi. Tools riducono il contesto rispetto a chiamate API raw. **Triade tool connection (1 min aggiuntivo se il tempo lo permette):** tre ruoli distinti — Skills per conoscenza statica (zero round-trip, caricamento on-demand), CLI per esecuzione con output strutturato JSON + exit codes semantici, MCP per ispezione dello stato live (cambia mentre l'agente lavora). La distinzione spiega perché il nostro setup usa CLI+scripts nelle skill invece di MCP per tutto: `skills → static knowledge | CLI → execution | MCP → live state`. 📊 Diagramma D3 da completare (aggiornamento `21-tool-connection-strategy.excalidraw`).
  ...
  9. Agents + Plugin — subagenti specializzati (Explore/Plan/general-purpose) + plugin come bundle. **→ Subagent anti-patterns (1 min — prima di Agent Teams):** tre errori comuni che azzzerano il valore dell'isolamento: (1) **stesso permission set del main thread** — il subagente può fare tutto quello che farebbe il thread principale, l'isolamento è solo contestuale non operativo; (2) **output format libero** — il main thread non riesce a usare il risultato, serve un formato fisso (JSON struct o Markdown con sezioni definite); (3) **dipendenze forti tra subtask** — se il subagente B ha bisogno dello stato intermedio del subagente A, non sono parallelizzabili: usare subagenti in sequenza con stato condiviso è più costoso e più lento di un singolo thread. **→ Agent Teams (30 sec — solo menzione):** "I subagenti lavorano dentro la vostra sessione e riportano al chiamante. Anthropic ha aggiunto una modalità sperimentale — Agent Teams — dove ogni lavoratore è un'istanza Claude Code indipendente: possono parlare tra loro, condividere una task list, coordinare lavoro parallelo. Non lo usiamo oggi perché le primitive vengono prima — ma è la direzione." (Docs: [Agent Teams](https://docs.anthropic.com/en/docs/claude-code/agent-teams) · [Sub-agents](https://docs.anthropic.com/en/docs/claude-code/sub-agents))
...
- **Demo artifact:** Continuazione della pipeline avviata nel Cold Open. Il workflow è spec-driven: si decide cosa fare (spec/storia), poi si delega l'esecuzione.
  ...
  5. **Git worktrees — demo motivata (5 min):** 📊 `ai-dev/course/diagrams/excalidraw/20-git-worktrees.excalidraw` — Prima il problema: lanci due subagenti sullo stesso working tree, si pestano i piedi su file/branch. Poi la soluzione: `git worktree add ../repo-feature feature/auth` → ogni agente lavora in isolamento completo, branch separata, zero conflitti. Mostra `git worktree list`. Poi il collegamento nativo: `isolation: "worktree"` nel tool Agent di Claude Code — il worktree viene creato e ripulito automaticamente. Nota sul pattern avanzato: Replit Agent 4 ha portato questo all'estremo — lancia N sessioni parallele su worktree separati, valuta i risultati, tiene la migliore. Con `isolation: "worktree"` e un orchestrator puoi replicare lo stesso approccio. **→ Agent Teams (30 sec):** "L'isolamento con worktree risolve il conflitto su file e branch. Agent Teams fa il passo successivo: il lead assegna task, i teammate lavorano in worktree separati, si coordinano via task list condivisa, e il lead sintetizza i risultati. Il pattern Replit — N sessioni parallele, valuta, tieni la migliore — con Agent Teams si fa nativamente." Headless mode (`claude -p`, `codex exec`) come cenno finale (30 sec).
